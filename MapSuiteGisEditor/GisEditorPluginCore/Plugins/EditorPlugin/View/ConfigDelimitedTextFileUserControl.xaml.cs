...
{
    /// <summary>
    /// Interaction logic for ConfigDelimitedTextFileUserControl.xaml
    /// </summary>
    public partial class ConfigDelimitedTextFileUserControl : CreateFeatureLayerUserControl
    {
        private ConfigLayerStructureViewModel configLayerStructureViewModel;

        public ConfigDelimitedTextFileUserControl(FeatureLayer featureLayer)
        {
            InitializeComponent();

            if (EditorUIPlugin.IsRelateAndAliasEnabled)
            {
                GridView.Columns.Insert(GridView.Columns.Count - 1, (GridViewColumn)Resources["AliasColumn"]);
            }
