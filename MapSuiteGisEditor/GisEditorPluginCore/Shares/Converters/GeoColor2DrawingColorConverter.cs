...
{
    [Serializable]
    public class GeoColor2DrawingColorConverter : ValueConverter
    {
        public override object Convert(object value, Type targetType, object parameter, System.Globalization.CultureInfo culture)
        {
            if (value is GeoColor)
            {
                return Convert((GeoColor)value);
            }
            else if (value is Color)
            {
                return ConvertBack((Color)value);
            }
            else
            {
        ...
        {
            return GeoColor.FromArgb(mediaColor.A, mediaColor.R, mediaColor.G, mediaColor.B);
        }
    }
}