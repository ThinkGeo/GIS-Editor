...
{
    [Serializable]
    public class MergeWizardSaveResultsStep : WizardStep<MergeWizardShareObject>
    {
        [NonSerialized]
        private MergeWizardSaveResultsUserControl content;
        private MergeWizardShareObject mergeEntity;

        public MergeWizardSaveResultsStep()
        {
            ...
            Description = GisEditor.LanguageManager.GetStringResource("ToolsMergeWizardStepThreeSaveTheResults");
        }

        protected override void EnterCore(MergeWizardShareObject parameter)
        {
            mergeEntity = parameter;
            content = new MergeWizardSaveResultsUserControl(parameter);
            Content = content;
            content.DataContext = parameter;
            base.EnterCore(parameter);
            parameter.TempFileName = FolderHelper.GetWizardTempFileName(parameter.SelectedLayers[0].Name, parameter.WizardName);
        }

        protected override bool CanMoveToNextCore()
        {
            var viewModel = content.DataContext as MergeWizardShareObject;
            bool result = false;
            if (viewModel != null)
            {
        ...
        protected override bool LeaveCore(MergeWizardShareObject parameter)
        {
