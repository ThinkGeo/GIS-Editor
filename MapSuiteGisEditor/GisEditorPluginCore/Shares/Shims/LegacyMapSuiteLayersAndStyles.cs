...
namespace ThinkGeo.Core
{
    ...
    public class WorldMapKitLayer : ThinkGeoRasterMapsAsyncLayer
    {
        private WorldMapKitMapType mapType;
        private WorldMapKitProjection projection;

        public WorldMapKitLayer()
            : base()
        {
        ...
        public WorldMapKitLayer(string clientId, string privateKey, WorldMapKitMapType mapType)
            : base(clientId, privateKey)
        {
            MapType = mapType;
        }
        ...
        public new WorldMapKitMapType MapType
        {
            get
            {
                if (mapType == default)
                {
                    mapType = FromCloudMapType(base.MapType);
                }
                return mapType;
            }
            set
            {
                mapType = value;
                base.MapType = ToCloudMapType(value);
            }
        }

        public new WorldMapKitProjection Projection
        {
            get => projection;
            set
            {
                projection = value;
                base.Projection = projection == WorldMapKitProjection.DecimalDegrees
                    ? new Projection(Proj4Projection.GetWgs84ParametersString())
                    : new Projection(Proj4Projection.GetGoogleMapParametersString());
            }
        }
        ...
        private static ThinkGeoCloudRasterMapsMapType ToCloudMapType(WorldMapKitMapType type)
        {
            if (Enum.TryParse(type.ToString(), out ThinkGeoCloudRasterMapsMapType mapType))
            {
                return mapType;
            }
        ...
        private static WorldMapKitMapType FromCloudMapType(ThinkGeoCloudRasterMapsMapType type)
        {
            if (Enum.TryParse(type.ToString(), out WorldMapKitMapType mapType))
            {
                return mapType;
            }

            return WorldMapKitMapType.Default;
        }
    }
